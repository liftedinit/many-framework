...
pub struct AbciApp {
    ...
    omni_url: reqwest::Url,
}

impl AbciApp {
    /// Constructor.
    pub fn new<U>(omni_url: U) -> Result<Self, String>
    where
        U: IntoUrl,
    {
        Ok(Self {
            client: reqwest::blocking::Client::new(),
            omni_url: omni_url.into_url().map_err(|e| e.to_string())?,
            omni_client: OmniClient::new()
        })
    }
}
...
impl Application for AbciApp {
    fn info(&self, request: RequestInfo) -> ResponseInfo {
        ...
        self.client.post(&self.omni_url).body(request.).send()
