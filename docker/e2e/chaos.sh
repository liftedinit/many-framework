...
c=0
no_sleep () {
    echo -e "${GREEN}RUN TENDERMINT NO SLEEP CHAOS TEST ${GREEN}"
    while [ $c -le 14 ]
    do
...
rapid_fire () {
    echo -e "${RED}RUN TENDERMINT RAPID FIRE CHAOS ${RED}"
    while [ $c -le 30 ]
    do
...
random_time () {
    echo -e "${PURPLE}RUN TENDERMINT CHAOS TEST WITH RANDOM INTERVALS ${PURPLE}"
    while [ $c -le 45 ]
    do
    echo "${RED}STOP TENDERMINT CHAOS TEST $c times${RED}"
    ((c++))
