...
main() {
  ...
  tmux new-window -t "$tmux_name" -n ledger "./target/debug/omni-ledger --abci --addr 127.0.0.1:8001 --pem ~/Identities/id1.pem --state ./staging/ledger_state.json --persistent \"$root_dir/ledger.db\" 2>&1 | tee \"$root_dir/omni-ledger.log\""
