...
main() {
  ...
  tmux new-window -t "$tmux_name" -n ledger "./target/debug/many-ledger -v -v --abci --addr 127.0.0.1:8001 --pem $HOME/Identities/id1.pem --state ./staging/ledger_state.json --snapshots \"$root_dir/\" --persistent \"$root_dir/ledger.db\" 2>&1 | tee \"$root_dir/many-ledger.log\""
